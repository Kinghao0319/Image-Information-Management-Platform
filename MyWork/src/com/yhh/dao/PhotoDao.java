...
public class PhotoDao {
    ...
    *这个函数的功能是获取前端的数据集合，将文件打包成File以便后续操作
    ...
    public static List<FileItem> getRequsetFileItems(HttpServletRequest request,ServletContext servletContext){
        ...
        if(isMultipart) {
        	System.out.println("执行PhotoDao正常");
        	...
        	System.out.println("执行PhotoDao失败");
            return null;
        }
    }
    /**
    ...
    *这个函数的功能是将文件传到预先设置的绝对路径中，也就是项目里的imgs文件夹
     */
    public static boolean saveFile(FileItem item,String fileName,String path) {        
        //File savePath=new File("F:\\myecliwork\\MyWork\\WebRoot\\myImage");
